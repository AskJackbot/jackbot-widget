...
(function(config) {
  ...
  const defaultConfig = {
    // API Configuration
    apiEndpoint: 'http://127.0.0.1:5000/api/chat', // Updated to use localhost:5000
    corsProxyUrl: null, // CORS proxy URL, set to null if not needed
    model: 'sonar', // Default model
    useStream: false,
    
    // UI Configuration
    title: 'Jackbot AI',
    titleColor: '#FFFFFF', // Default title color (white)
    subtitle: 'Powered by @askjackbot',
  ...
  // Create a messages array for API
  function createMessagesArray(userMessage) {
    // Get the last 5 messages from history
    const history = chatHistory.load();
    const last5Messages = history.slice(-5);
    
    // Print the last 5 messages to console
    console.log('Last 5 messages:');
    last5Messages.forEach(msg => {
      console.log(`${msg.isUser ? 'User' : 'Assistant'}: ${msg.message}`);
    });
    ...
    // Add the last 5 messages to the request
    last5Messages.forEach(msg => {
      messages.push({
  ...
  function onUserRequest(message) {
    ...
    const messages = createMessagesArray(message);
    ...
    .then(data => {
      ...
      const response = data.response || data.message || data.content || 'No response received';
